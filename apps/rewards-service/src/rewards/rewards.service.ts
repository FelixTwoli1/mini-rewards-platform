...
import { RewardAccount, RewardTransaction } from '@prisma/client';
...
export class RewardsService {
  constructor(
    ...
    private readonly strategyExecutor: RewardStrategyExecutor
  ) {}

  async getBalance(userId: string): Promise<RewardAccount> {
  ...
  async awardPoints(dto: AwardPointsDto): Promise<{
    ...
    const result = this.strategyExecutor.execute(dto.eventType, {
      userId: dto.userId,
    ...
    logger.info(
      { userId: dto.userId, points: result.points, reason: result.reason },
      'Points awarded'
    );
  ...
  async getTransactionHistory(
    ...
    skip = 0,
    take = 20
  ): Promise<{ data: RewardTransaction[]; total: number; balance: number }> {
    const account = await this.prisma.rewardAccount.findUnique({ where: { userId } });
    if (!account) throw new NotFoundException('Reward account not found');
